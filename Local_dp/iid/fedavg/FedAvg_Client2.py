...
FILE_PATH = "./Data/iid_part_two.csv" 
LABEL_FLIP_PERCENTAGE = 0.2


def load_and_preprocess_data(file_path,labelflip_percentage=0.0):
    ...
    if labelflip_percentage > 0.0:
        num_samples = len(y)
        num_to_flip = int(labelflip_percentage * num_samples)
        flip_indices = np.random.choice(num_samples, size=num_to_flip, replace=False)

        unique_labels = np.unique(y)
        for idx in flip_indices:
            original_label = y[idx]
            flipped_choices = [l for l in unique_labels if l != original_label]
            y[idx] = np.random.choice(flipped_choices)
...
def client_fn2(context: Context):
    x_train, x_test, y_train, y_test = load_and_preprocess_data(FILE_PATH,labelflip_percentage=LABEL_FLIP_PERCENTAGE)
...
if __name__ == "__main__":
    app = ClientApp(client_fn=client_fn2)
