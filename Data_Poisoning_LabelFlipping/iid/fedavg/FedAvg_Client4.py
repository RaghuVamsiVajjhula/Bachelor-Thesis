...
#     parser.add_argument('--file', type=str, required=True, help='Path to the CSV file or folder for this client')
...
FILE_PATH = "./Data/iid_part_four.csv" 
...
def client_fn4(context: Context):
    x_train, x_test, y_train, y_test = load_and_preprocess_data(FILE_PATH)
    model = build_lstm_model((1, x_train.shape[2]), y_train.shape[1])
    client = LSTMClient(model, x_train, x_test, y_train, y_test)
    return client
...
if __name__ == "__main__":
    app = ClientApp(client_fn=client_fn4)
